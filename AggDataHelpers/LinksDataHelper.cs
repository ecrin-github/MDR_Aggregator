...
public class LinksDataHelper
{
    ...
    public LinksDataHelper(string aggsConnString, ILoggingHelper loggingHelper)
    {
        _aggsConnString = aggsConnString;
    ...
    public void SetUpTempPreferencesTable(IEnumerable<Source> sources)
    {
        ...
        var ds = sources.Select(s => new DataSource(s.id, s.preference_rating, s.database_name!)).ToList();
        conn.Open(); 
        ...
        sql_string = """
                     DROP TABLE IF EXISTS nk.dutch_id_checker;
    ...
    public IEnumerable<StudyLink> FetchLinks(int thisSourceId, string sourceConnString)
    {
        using var conn = new NpgsqlConnection(sourceConnString);
        var sql_string = $"""
                          select {thisSourceId} as source_1, 
                                          sd_sid as sd_sid_1, 
    ...
    public IEnumerable<OldNewLink> GetOldAndNewIds(string sourceConnString, int idType)
    {
        using var conn = new NpgsqlConnection(sourceConnString);
        var sql_string = $"""
                          select sd_sid as new_id, 
    ...
    public void CleanDutchSecIds()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        const string sqlString = """
                                 update nk.temp_study_links_collector 
    ...
    public void TidyIds1()
    {
        ...
        var sql_string = """
                         DELETE from nk.temp_study_links_collector
    ...
    public void TidyIds2()
    {
        ...
        sql_string = """
                     insert into nk.temp_study_links_collector
    ...
    public void AddAdditionalLinksUsingIdenticalSponsorIds()
    {
        ...
        var res = conn.ExecuteScalar<int>(sql_string);
        ...
        sql_string = """
                     delete from nk.temp_ids_of_interest t
        ...
        sql_string = """
                     drop table if exists nk.temp_pairs_from_last_agg;
        ...
        sql_string = """
                     insert into nk.temp_study_links_collector(source_1, sd_sid_1, sd_sid_2, source_2)
    ...
    public void TransferLinksToSortedTable()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = """
                         INSERT INTO nk.temp_study_links_sorted(
        ...
        var res1 = conn.Execute(sql_string);
        ...
        var res2 = conn.Execute(sql_string);
    ...
    public void ObtainStudyIds(int sourceId, string sourceConnString, PostgreSQLCopyHelper<IdChecker> copyHelper)
    {
        // Get a list of study ids from the source, so secondary ids can be checked against it.
        
        using var agg_conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = """
                         DROP TABLE IF EXISTS nk.temp_id_checker;
                                             CREATE TABLE nk.temp_id_checker (sd_sid VARCHAR) 
                         """;
        agg_conn.Execute(sql_string);

        using var srce_conn = new NpgsqlConnection(sourceConnString);
        sql_string = @"select sd_sid from ad.studies";
        var ids = srce_conn.Query<IdChecker>(sql_string);
        
        agg_conn.Open();
        copyHelper.SaveAll(agg_conn, ids);

        sql_string = "SELECT COUNT(*) FROM nk.temp_id_checker";
        var res = agg_conn.ExecuteScalar<int>(sql_string);
        _loggingHelper.LogLine($"Source {sourceId}: {res} sd_sids in total");
    }

    public void CheckIdsAgainstSourceStudyIds(int sourceId)
    {
        // Carry out the check on secondary ids and mark then as valid or invalid.
        
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = $"""
                          UPDATE nk.temp_distinct_links t
                                         SET valid = false 
                                             FROM 
                                       ...
                                       SET valid = false 
                                           FROM
                                              (SELECT k.preferred_sd_sid 
                      					     ...
                      					     WHERE k.preferred_source_id = {sourceId}
                      					     AND s.sd_sid is null) invalids
    ...
    public void CreateGroupedStudiesTable()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        const string sqlString = """
                                 DROP TABLE IF EXISTS nk.linked_study_groups;
    ...
    private void Identify1ToNGroupedStudies()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = """
                         DROP TABLE IF EXISTS nk.temp_multilinked_studies;
        ...
        sql_string = """
                     INSERT into nk.temp_multilinked_studies
    ...
    private void IdentifyNToNGroupedStudies()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = """
                         Update nk.temp_multilinked_studies ks1
                                     SET complex = true
    ...
    private void Extract1ToNGroupedStudies()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = """
                         INSERT INTO nk.linked_study_groups
    ...
    private void ExtractNToNGroupedStudyData()
    {
        ...
        List<ComplexStudy> complex_studies = [];
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = """
                         select group_source_id, group_sd_sid, 
                                                       member_source_id, member_sd_sid 
                                                       ...
                                                       where complex = true
                         """;
        var complex_rows = conn.Query<ComplexStudyRow>(sql_string).ToList();
        if (complex_rows.Count != 0)
        {
            var next_group_num = 1;
            foreach (var r in complex_rows)
            {
                ...
                else
                {
                    ...
                    var group_num = (from cs in complex_studies where r.group_source_id == cs.src_id && r.group_sd_sid == cs.sid_id select cs.group_number).FirstOrDefault();
                    if (group_num == 0)
                    {
                        foreach (var cs in complex_studies.Where(cs => r.member_source_id == cs.src_id && r.member_sd_sid == cs.sid_id))
                        {
            ...
            var complex_studies2 = complex_studies.Select(i => i)
            ...
            for (var g = 1; g <= next_group_num; g++)
            {
                var study_group =
                    complex_studies2.Select(i => i).Where(i => i.group_number == g).ToList();

                if (study_group.Count == 0) continue;
                {
                    ...
                    var study_num = study_group.Count;
                    for (var i = 0; i < study_num - 1; i++)
                    {
                        var s = study_group[i];
                        for (var j = i + 1; j < study_num; j++)
                        {
                            var t = study_group[j];
            ...
            if (links.Count > 0)
            {
                ...
                var feedback = $"{links.Count} relationship records added as part of n to n study relationships";
        ...
        var res1 = conn.ExecuteScalar<int>(sql_string);
            
        sql_string = """
                     DROP TABLE IF EXISTS nk.linked_study_groups2;
        ...
        sql_string = "SELECT COUNT(*) FROM nk.linked_study_groups";
        var res2 = conn.ExecuteScalar<int>(sql_string);
        var diff = res1 - res2;
    ...
    private void DeleteGroupedStudyLinkRecords()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = """
                         DELETE FROM nk.temp_distinct_links k
    ...
    public void AddMissingLinks()
    {
        ...
        sql_string = """
                     DROP TABLE IF EXISTS nk.temp_missing_links;
        ...
        sql_string = """
                     INSERT INTO nk.temp_distinct_links
                                      (source_id, sd_sid, preferred_sd_sid, preferred_source_id)
    ...
    public void CascadeLinks()
    {
        ...
        using var conn = new NpgsqlConnection(_aggsConnString);
        var match_number = 500;  // arbitrary start number
        while (match_number > 0)
        {
            ...
            if (match_number <= 0) continue; // do the update
            sql_string = """
                         UPDATE nk.temp_distinct_links t1
    ...
    private void MakeLinksDistinct()
    {
        using var conn = new NpgsqlConnection(_aggsConnString);
        var sql_string = "SELECT COUNT(*) FROM nk.temp_distinct_links";
        var res1 = conn.ExecuteScalar<int>(sql_string);
            
        sql_string = """
                     DROP TABLE IF EXISTS nk.temp_distinct_links2;
